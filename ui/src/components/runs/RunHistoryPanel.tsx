  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-1">
        <span className="text-xs font-semibold text-gray-600">Run History</span>
        <button
      ...
      {runs.length === 0 ? (
        <p className="py-4 text-center text-xs text-gray-400">No runs yet</p>
      ) : (
        <div className="space-y-1">
          {runs.map((run) => (
            <button
              ...
              className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left text-xs hover:bg-gray-100"
            >
              <RunStatusBadge status={run.status} />
              <span className="flex-1 truncate text-gray-600">#{run.id}</span>
